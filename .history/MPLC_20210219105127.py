...
class wave():
    ...
    def gaussianBeam(self, amplitude:float, sigma:float):
        for i in range(self.area[0]):
            for j in range(self.area[1]): 
                self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2)

    def gaussianBeamOffCentreOne(self, amplitude:float, sigma:float):
            for i in range(self.area[0]):
                for j in range(self.area[1]): 
                    self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)/3)*pix_size)/np.sqrt(2)*sigma)**2)
    
    def gaussianBeamOffCentreTwo(self, amplitude:float, sigma:float):
            for i in range(self.area[0]):
                for j in range(self.area[1]): 
                    self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)*2/3)*pix_size)/np.sqrt(2)*sigma)**2)
    
    #Gaussian beam with pi phase difference between beam on left and beam on right
    def doubleGaussOne(self, amplitude, sigma):
        for i in range(self.area[0]):
            for j in range(self.area[1]):
                self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)/3)*pix_size)/np.sqrt(2)*sigma)**2) + np.exp(-(((i-self.area[0]*(512/1024)/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)*2/3)*pix_size)/np.sqrt(2)*sigma)**2)
                if(j < self.area[1]/2):
    ...
    def doubleGaussTwo(self, amplitude, sigma):
        for i in range(self.area[0]):
            for j in range(self.area[1]):
                self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*(512/1024)*1/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)*1/3)*pix_size)/np.sqrt(2)*sigma)**2) + np.exp(-(((i-self.area[0]*(512/1024)*1/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*(512/1024)*2/3)*pix_size)/np.sqrt(2)*sigma)**2)
    ...
    def initScreens(self, f:float):
        ...
        for i in range(self.area[0]):
            for j in range(self.area[1]):
                ...
                lens[i][j] = (2*np.pi/self.wavelength) * (f - pow((f**2 + ((i - self.area[0]*pix_size)**2 + ((j - self.area[1]/2)*pix_size)**2), 0.5))
...
phase_array = mulitplane_phase_array(3, int_plan_dist, input_modes, output_modes)
...
test_wave.doubleGaussTwo(1.0, 20)
...
test_wave2.doubleGaussTwo(1.0, 20)
