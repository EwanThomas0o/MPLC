...
class wave():
    ...
    def doubleGaussTwo(self, amplitude, sigma):
        for i in range(self.area[0]):
            for j in range(self.area[1]):
                self.field[i][j] = amplitude * np.exp(-(((i-self.area[0]*1/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*1/3)*pix_size)/np.sqrt(2)*sigma)**2) + np.exp(-(((i-self.area[0]*1/2)*pix_size)/np.sqrt(2)*sigma)**2 -(((j-self.area[1]*2/3)*pix_size)/np.sqrt(2)*sigma)**2)
...
input_wave1.doubleGaussOne(1.0, 30)
...
input_wave2.doubleGaussTwo(1.0, 30)

input_modes = [input_wave1, input_wave2]

output_wave1 = wave((n,n), wavelength)
output_wave1.gaussianBeamOffCentreOne(1.0, 30)
output_wave2 = wave((n,n), wavelength)
output_wave2.gaussianBeamOffCentreTwo(1.0, 30)

output_modes = [output_wave1, output_wave2]
# output_wave.gaussianBeam(1.0, 30)
...
test_wave.doubleGaussOne(1.0, 30)
# test_wave.gaussianBeam(1.0,30)
...
test_wave2.doubleGaussOne(1.0, 30)
# test_wave2.gaussianBeam(1.0,30)
...
animation = AngularSpectrum(test_wave, test_wave2, phase_array, 41, dist)
