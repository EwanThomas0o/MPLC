    ...
    def doubleGaussOne(self, amplitude, sigma):
        for i in range(self.area[0]):
            for j in range(self.area[1]):
                ...
                if( i < self.area[0]/2 and j < self.area[1]/2):
...
phase_array.screens = np.load('mplc.dat') #Dont forget to uncomment this and comment out the while below when you want to use data from old cycle
...
animation = AngularSpectrum(test_wave, test_wave2, phase_array, 80, dist)
SaveAndDisplay(animation, "propagation_new.mp4", 30)

# plt.figure(0)
# plt.imshow(phase_array.screens[1], cmap='hot')
# plt.colorbar().set_label("Phase")
# plt.xlabel("x ($\mu$m)")
# plt.ylabel("y ($\mu$m)")
# plt.title("Phase of phase screen 1 in array")
# plt.show()
